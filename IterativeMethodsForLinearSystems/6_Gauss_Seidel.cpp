...
std::vector<double> Gauss_Seidel_iterative_method(std::vector<std::vector<double>> A, std::vector<double> B, 
...
int main(){
    ...
    if (!checkDiagonalDominant(A)) {
        ...
        std::vector<double> p = Gauss_Seidel_iterative_method(A, B, p0);
        std::cout << "Solution: ";
        print_vector(p);
    }
